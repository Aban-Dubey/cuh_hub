...
axios.defaults.baseURL = "https://cuh-hub-server.vercel.app/";
...
export async function authenticate(username){
    try {
        return axios.post('/api/user/authenticate', { username });
    } catch (error) {
...
export async function updateUser(response){
    try {
        ...
        const data = await axios.put('/api/user/updateUser', response, { headers: { "Authorization": `Bearer ${token}`}});
...
export async function generateOTP(username) {
    try {
        ...
        if(status === 201){
            ...
            await axios.post('/api/user/registerMail', { username, userEmail: email, text, subject: "Password Recovery"});
        }
...
export async function verifyOTP({ username, code }){
    try {
        const { data, status } = await axios.get('/api/user/verifyOTP', {params: { username, code }});
        return { data, status };
    } catch (error) {
